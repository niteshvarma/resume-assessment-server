...
CREATE_ROUTE_RESULT=$(aws apigatewayv2 create-route --api-id "$API_ID" --route-key "POST /$RESOURCE_NAME")

# Exit if route creation fails
if [ $? -ne 0 ]; then
  echo "Error: Failed to create route for $RESOURCE_NAME"
  exit 1
fi
...
echo "Route ID for /$RESOURCE_NAME: $ROUTE_ID"

# Create HTTP Proxy integration for the resource
echo "Creating HTTP Proxy integration for $RESOURCE_NAME..."
CREATE_INTEGRATION_RESULT=$(aws apigatewayv2 create-integration --api-id "$API_ID" --integration-type HTTP_PROXY \
  --integration-method POST --integration-uri "http://$EC2_IP:$PORT/$RESOURCE_NAME" --payload-format-version "1.0")

# Exit if integration creation fails
if [ $? -ne 0 ]; then
  echo "Error: Failed to create integration for $RESOURCE_NAME"
  exit 1
fi
...
echo "Setting integration target for /$RESOURCE_NAME route..."
...
if [ $? -ne 0 ]; then
  echo "Error: Failed to update route for $RESOURCE_NAME"
  exit 1
fi
...
aws apigatewayv2 create-route-response --api-id "$API_ID" --route-id "$OPTIONS_ROUTE_ID" --route-response-key "default" \
  --response-parameters '{"method.response.header.Access-Control-Allow-Origin": {"Required": "*"}, 
                          ...
                          "method.response.header.Access-Control-Allow-Methods": {"Required": "POST,OPTIONS"}}'

# Exit if CORS setup fails
if [ $? -ne 0 ]; then
  echo "Error: Failed to set up CORS for $RESOURCE_NAME"
  exit 1
fi

echo "CORS setup complete for /$RESOURCE_NAME."
